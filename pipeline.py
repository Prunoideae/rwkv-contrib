...
class StatefulPipeline(Generic[T]):
    ...
    def push(self, ctx: T, args: GenerationArgs = None, rnn_mode=False) -> None:
        """
        ...
        if args.linear_mode:
            ...
            for token in tokens:
                self.last_token, self.state = self.infer([token], args=args)
        else:
            self.last_token, self.state = self.infer(self.encode(ctx), args=args)

    def sample_logits(self, logits: Tensor, args: GenerationArgs = None):
        args = args or self.default_args
    ...
    def infer(self, tokens: List[int], args: GenerationArgs = None) -> tuple[Optional[int], Any]:
        """
    ...
    def _generate(self, ctx: List[int], generation_length: int = 100, *, args: GenerationArgs = None) -> Generator[List[int], None, None]:
        args = args or self.default_args

        tokens_tmp = []
        if args.linear_mode:
            for ct in ctx:
                token, self.state = self.infer([ct], args=args)
        else:
    ...
    def generate(self, ctx: T, generation_length: int = 100, *, args: GenerationArgs = None) -> Generator[T, None, None]:
...
class RecallablePipeline(StatefulPipeline[T]):
    ...
    def _generate(self, ctx: List[int], generation_length: int = 100, *, args: GenerationArgs = None) -> Generator[Any, None, None]:
        self.history.append(deepcopy(self.state))
        self.history_tokens.append(self.last_token)
        self.history_penalties.append(self.penalty.copy())
